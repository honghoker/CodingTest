...
var r = Array(repeating: Array(repeating: [Int](), count: N), count: N)
...
for k in 0..<N {
    for i in 0..<N {
        for j in 0..<N where A[i][k] != Int.max && A[k][j] != Int.max && A[i][k] + A[k][j] < A[i][j] {
            A[i][j] = A[i][k] + A[k][j]
            r[i][j] = (r[i][k].dropLast()) + r[k][j]
        }
    }
}
print(A.map { $0.map { String($0 == Int.max ? 0 : $0) }.joined(separator: " ") }.joined(separator: "\n"))
print(r.map { $0.map { "\($0.count) \($0.map { String($0 + 1) }.joined(separator: " "))" }.joined(separator: "\n") }.joined(separator: "\n"))
